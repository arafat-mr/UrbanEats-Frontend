...
export default function MyOrders() {
  ...
  const fetchOrders = async () => {
    setLoading(true);
    try {
      const res = await fetch("https://urban-eats-backend.vercel.app/api/providers/my-orders", {
        credentials: "include",
      });
  ...
  const handleStatusUpdate = async (orderId: string, newStatus: Order["orderStatus"]) => {
    try {
      const res = await fetch(`https://urban-eats-backend.vercel.app/api/providers/orders/update/${orderId}`, {
        method: "PATCH",
